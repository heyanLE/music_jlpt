...
class ForegroundRenderer:
    ...
    def render(self, frame_id: str | None, active_index: int | None, output: Path | None, cover_only: bool = False, cover_visible: bool = True, countdown_value: int | None = None, *, lyric_only: bool = False, lyric_x_override: float | None = None) -> Image.Image:
        ...
        text_width = draw.textbbox((0, 0), text, font=lyric_font)[2]; lyric_x = (self.width - text_width) / 2
        ...
        if not lyric_only and self.learning_aids["neighbors"]["enabled"] and frame_id in self.ordered_ids:
            ...
            for adjacent, direction in ((position - 1, -1), (position + 1, 1)):
                ...
                neighbor_font = self.fit(draw, neighbor_text, 70, 53, self.px(1728, "x"))
                neighbor_width = draw.textbbox((0, 0), neighbor_text, font=neighbor_font)[2]
        ...
        for index, part in enumerate(parts):
            value = part["text"]
            self.outlined(draw, (cursor_x, self.px(330, "y")), value, lyric_font, active if index == active_index else body, 4)
            cursor_x += draw.textbbox((0, 0), value, font=lyric_font)[2]
        ...
        for ruby in frame["caption"].get("furigana", []):
            ...
            ruby_x = lyric_x + draw.textbbox((0, 0), text[:start], font=lyric_font)[2]
            self.outlined(draw, (ruby_x, self.px(295, "y")), ruby["reading"], ruby_font, active if is_active(start, end) else body, 2)
        ...
        for card_index, card in enumerate(frame.get("grammarCards", [])):
            ...
            token_x = lyric_x + draw.textbbox((0, 0), text[:start], font=lyric_font)[2]
            token_width = draw.textbbox((0, 0), token, font=lyric_font)[2]
