...
LOANWORDS = {"グラス": "glass", "フレイバー": "flavor"}
KKS = kakasi()
...
def furigana_for_card(card: dict, lyric: str, search_start: int) -> tuple[list[dict], int]:
    ...
    if token_pos < 0 or len(runs) != 1:
        return [], next_search
    run = runs[0]
    reading = card["reading"]
    prefix, suffix = token[:run.start()], token[run.end():]
    if prefix and KANA_RE.fullmatch(prefix) and reading.startswith(kata_to_hira(prefix)):
        reading = reading[len(prefix):]
    if suffix and KANA_RE.fullmatch(suffix) and reading.endswith(kata_to_hira(suffix)):
...
def export_review(frames: list[dict]) -> Path:
    ...
    for frame in frames:
        ...
        lines.append("")
    path = ROOT / "deliverables" / "review" / "kanjou-glass-review.md"
    path.parent.mkdir(parents=True, exist_ok=True)
