#!/usr/bin/env python3
"""Render the non-authorizing setup preview for the three-video project."""
...
def main() -> None:
    ...
    renderer.frames[frame_id]["grammarCards"] = [
        {"token": "喉", "reading": "のど", "romaji": "nodo", "zhMeaning": "喉咙", "grammarStructureZh": "名词"},
        {"token": "通った", "reading": "とおった", "romaji": "tootta", "zhMeaning": "通过；滑过", "grammarStructureZh": "动词过去式"},
        {"token": "フレイバー", "reading": "", "romaji": "fureibaa", "sourceWord": "flavor", "zhMeaning": "风味；滋味", "grammarStructureZh": "片假名名词"},
        {"token": "に", "reading": "", "romaji": "ni", "functionZh": "标记作用对象", "grammarStructureZh": "格助词"},
        {"token": "は", "reading": "", "romaji": "wa", "functionZh": "提示主题", "grammarStructureZh": "系助词"}
    ]
    ...
    report = {
        ...
        "layers": ["three-video-sequential-background", "foreground", "persistent-transparent-spectrum"],
        "foregroundTemplate": "study-current-v3",
        "sampleCards": "layout-only placeholders; not accepted linguistic content",
        "checks": {
            "embeddedCoverVisible": True,
            "magicColorApplied": True,
            "foregroundTextBold": True,
            "kanjiOnlyFurigana": True,
            "loanwordSourceAnnotation": True,
            "romajiAnchoredPerToken": True,
            "lineTranslation": True,
            "singleHorizontalCardRow": True,
            "transparentPersistentSpectrum": True,
            "sourceAudioMuted": True,
            "masterFlacPreserved": True
        },
        "sceneTimeline": scene,
    ...
    subprocess.run([
        sys.executable, str(SKILL / "scripts" / "validate_project.py"), str(ROOT), "--stage", "setup"
    ], check=True)
    state = load(PROJECT / "build-state.json")
    state["stage"] = "setup_complete"
    state["renderAuthorization"] = False
    state.setdefault("notes", []).append(
        "Setup complete: three sequential video clips, persistent transparent spectrum, extracted cover palette, and Hi-Res covers."
    )
    write(PROJECT / "build-state.json", state)
    print(json.dumps({"preview": str(final_png), "stage": state["stage"]}, ensure_ascii=False))
