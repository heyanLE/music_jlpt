"""Clean short render of the horizontal main-lyric rail from source assets.
...
def main():
    ...
    for number in range(count):
        ...
        foreground_visible = now >= ordered[0]["startMs"] and not (
            now < 96114 and now >= min(frame["endMs"] + 500, next_start)
        )
        ...
        for index in range(max(0, current - 2), min(len(ordered), current + 3)):
            ...
            for card_index, card in enumerate(item.get("grammarCards", [])):
    ...
    (OUT / "preview.json").write_text(json.dumps({
        "sourceVideo": "source/background.mp4", "sourceAudio": "source/music.flac",
