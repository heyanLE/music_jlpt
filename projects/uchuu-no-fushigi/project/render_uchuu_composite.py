...
ROOT=Path(__file__).parent; OUT=ROOT/'output'; data=json.loads((ROOT/'uchuu.frames.approved.json').read_text(encoding='utf8')); qrc=json.loads((OUT/'uchuu-qm.parsed.json').read_text(encoding='utf8'))
W,H=1920,1080; DELAY=1000; font='C:/Windows/Fonts/msyhbd.ttc'; cover=Image.open(ROOT/'uchuu-cover.jpg').convert('RGBA'); audio=r'C:/Users/eke_l/Downloads/夢限大みゅーたいぷ - うちゅうのふしぎ.flac'; bg=r'C:/Users/eke_l/Desktop/TV动画_ED影像公开「バンドリ!_ゆめ∞みた(BanG_Dream!_YUME∞MITA)」.39577256487.mp4'
DIR=OUT/'uchuu_overlay_segments'; DIR.mkdir(exist_ok=True)
...
cf=OUT/'uchuu_overlay.concat.txt'; cf.write_text('\n'.join(concat),encoding='utf8')
target=OUT/'uchuu-qrc-composite.mp4'
