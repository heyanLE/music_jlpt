...
def state_key(state: dict) -> tuple:
    return state["kind"], state.get("frameId"), state.get("activePartIndex")
...
def main() -> None:
    ...
    for frame in frames:
        ...
        lines.append({"frameId": frame["id"], "startMs": start, "endMs": end, "parts": normalized_parts, "matchMode": match_mode})
    ...
    write(args.output, {"schemaVersion": 1, "clock": "output", "durationMs": args.duration_ms, "offsetMs": offset, "segments": segments, "matches": matches, "summary": summary})
