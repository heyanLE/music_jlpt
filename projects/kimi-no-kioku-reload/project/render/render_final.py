#!/usr/bin/env python3
"""Render the reviewed five-video project as background -> foreground -> overlay.
...
NEIGHBOR_HELPER = Path(__file__).with_name("render_static_neighbors_foreground.py")
NEIGHBOR_HELPER_SHA256 = "89e9e09ef53b7ca190eca0c7b9827aa797b3806b63aef22620e7862077d06249"
...
def main() -> None:
    ...
    if sha(NEIGHBOR_HELPER) != NEIGHBOR_HELPER_SHA256:
        raise RuntimeError("Pinned static-neighbor foreground helper changed")
    ...
    if manifest.get("pipelinePreset") != "custom" or len(scenes) != 5:
        raise SystemExit("This project renderer requires the documented five-scene custom preset")
    ...
    foreground_path = run / "foreground-static-neighbors.mov"
    if not foreground_path.exists():
        subprocess.run([
            sys.executable,
            str(NEIGHBOR_HELPER),
            "--start-ms", "0",
            "--end-ms", str(duration_ms),
            "--foreground-mov", str(foreground_path),
            "--static-neighbors",
        ], check=True)

    background_args, background_filter, background_count = background_inputs_and_filter(root, scenes, width, height, duration_ms)
    command = ["ffmpeg", "-y", "-v", "error", *background_args, "-i", str(foreground_path)]
    ...
    print(json.dumps({"candidate": str(candidate), "durationMs": duration_ms, "foreground": "static-neighbors", "spectrum": overlay_on}, ensure_ascii=False))
