...
W,H=1920,1080; DELAY=1000; font='C:/Windows/Fonts/msyhbd.ttc'; cover=Image.open(ROOT/'love2000-cover.jpg').convert('RGBA'); audio='C:/Users/eke_l/Music/遠野ひかる - LOVE 2000_EM.flac'; bg='C:/Users/eke_l/Desktop/【Hi-Res】LOVE2000败犬老八无字版【NCED1】.26084639254.mp4'
...
def wrap_card_text(d,t,f,max_width,max_lines=2):
 ...
 consumed=''.join(lines)
 if len(consumed)<len(str(t)):
  tail=lines[-1]
  while tail and d.textbbox((0,0),tail+'…',font=f)[2]>max_width: tail=tail[:-1]
  lines[-1]=tail+'…'
 return lines
...
def render(fr,active,path):
 ...
 if fr['kind']=='lyric':
  ...
  for card,w in zip(cards,widths):
   d.rounded_rectangle((x,705,x+w-gap,950),20,fill=(12,75,120,128),outline=(135,215,255,135),width=2); cx=x+(w-gap)/2
   b=d.textbbox((0,0),card['token'],font=F(34)); d.text((cx-(b[2]-b[0])/2,730),card['token'],font=F(34),fill='white')
   mf=F(24)
   for line_index,t in enumerate(wrap_card_text(d,card.get('functionZh',card['zhMeaning']),mf,w-gap-24)):
    b=d.textbbox((0,0),t,font=mf); d.text((cx-(b[2]-b[0])/2,790+line_index*29),t,font=mf,fill='white')
   b=d.textbbox((0,0),card['posZh'],font=F(23)); d.text((cx-(b[2]-b[0])/2,875),card['posZh'],font=F(23),fill='white')
   x+=w
 im.save(path)
concat=[]
...
target=OUT/'love2000-qrc-composite-v2.mp4'
