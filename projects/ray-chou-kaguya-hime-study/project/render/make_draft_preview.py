"""Render the largest fitting draft frame for structural review."""
...
for candidate in sorted(frames, key=lambda frame: len(frame.get("grammarCards", [])), reverse=True):
    try:
        renderer.render(candidate["id"], None, output); selected = candidate; break
    except RuntimeError:
        continue
if selected is None: raise RuntimeError("No draft frame fits the fixed one-row card layout")
report = {"result": "passed", "preview": output.relative_to(ROOT).as_posix(), "selectedFrame": selected["id"], "cardCount": len(selected["grammarCards"]), "checks": {"sharedTokenAnchors": True, "oneRowCards": True, "fixedMeaningWrap": "at-most-two-lines"}, "note": "Meanings, token boundaries and POS are drafts pending review."}
