...
ROOT=Path(__file__).parent; OUT=ROOT/'output'; DELIVERABLES=ROOT.parent/'deliverables'; data=json.loads((ROOT/'feel-my-soul.frames.approved.json').read_text(encoding='utf8')); qrc=json.loads((DELIVERABLES/'feel-qm.parsed.json').read_text(encoding='utf8'))
...
def ruby_parts(base, reading):
 ...
 return parts
def outlined(d, xy, text, font, fill='white', stroke=3):
 """Draw every non-card foreground text run with a readable black outline."""
 d.text(xy, text, font=font, fill=fill, stroke_width=stroke, stroke_fill=(0, 0, 0, 235))
def centered(d,y,t,f,fill='white',stroke=3):
 b=d.textbbox((0,0),t,font=f); outlined(d,((W-(b[2]-b[0]))/2,y),t,f,fill,stroke)
def clean_pos(t):
  ...
  if not ruby:
   ...
   for idx,(item,w) in enumerate(zip(ruby,widths)):
    ...
    for base_part, ruby_part in ruby_parts(item['base'],item['reading']):
     outlined(d,(part_x,360),base_part,mf,color,3)
     if ruby_part:
      outlined(d,(part_x,320),ruby_part,rf,color,2)
     part_x+=d.textbbox((0,0),base_part,font=mf)[2]
    r=item['romaji']; b=d.textbbox((0,0),r,font=F(34)); outlined(d,(x+(w-(b[2]-b[0]))/2,485),r,F(34),color,3); x+=w+gap
  centered(d,595,cap['translationZh'],F(46),(245,245,245,235),3); cards=fr['grammarCards']; n=max(1,len(cards)); total=1740; gap=12; widths=[total//n]*n; widths[-1]+=total-sum(widths); x=90
