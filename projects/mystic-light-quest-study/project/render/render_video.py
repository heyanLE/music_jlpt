...
SKILL_SCRIPTS = Path(__file__).resolve().parent / 'runtime'
...
class ForegroundRenderer:
    ...
    def render(self, frame_id: str | None, active_index: int | None, output: Path, cover_only: bool = False, cover_visible: bool = True) -> None:
        ...
        japanese_character_mode = self.presentation.get('highlight', {}).get('japaneseMode') == 'character'
        for index, value in enumerate(text):
            highlighted = is_active(index, index+1) or (not japanese_character_mode and any(a <= index < b for a,b in active_cards))
