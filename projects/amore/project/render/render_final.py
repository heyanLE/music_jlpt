...
RUN = P / "work" / "20260822-r03-background-onset-aligned"
OVER = RUN / "foreground"
W, H, FPS, AUDIO_OFFSET, DURATION = 1920, 1080, 30, 3183, 279156
...
def render_state(frame, parts, active_index, palette, path):
    ...
    if cards:
        ...
        for card in cards:
            ...
            tok=fit(draw,card["token"],37,16,inner); pos=fit(draw,card["posZh"],25,14,inner); meaning=card.get("functionZh",card.get("zhMeaning","")); mf=font(26); ml=wrap(draw,meaning,mf,inner)
            if len(ml) > 2: raise RuntimeError(f"Card meaning exceeds the approved two-line limit: {frame['id']} {card['token']}")
...
def main():
    ...
    for item_index,(frame,row,mode) in enumerate(items):
        ...
        parts=row["parts"]
        for index,part in enumerate(parts):
            start=part["startMs"] + AUDIO_OFFSET; end=start+part["durationMs"]
            if start>cursor:
    ...
    candidate=RUN/"amore--16x9--20260822-r03-background-onset-aligned.mkv"
    fc="[0:v]fps=30,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih):black,setpts=PTS-STARTPTS[bg];[1:v]fps=30,format=rgba,setpts=PTS-STARTPTS[sp];[2:v]fps=30,format=rgba,setpts=PTS-STARTPTS[fg];[bg][sp]overlay=36:894:format=auto[bgs];[bgs][fg]overlay=0:0:format=auto,fps=30,format=yuv420p[v]"
    cmd=["ffmpeg","-y","-v","error","-stream_loop","-1","-i",str(S/"background.mp4"),"-i",str(spectrum),"-f","concat","-safe","0","-i",str(concat_path),"-itsoffset",f"{AUDIO_OFFSET/1000:.6f}","-i",str(S/"music.flac"),"-filter_complex",fc,"-map","[v]","-map","3:a:0","-t",f"{DURATION/1000:.6f}","-c:v","libx264","-crf","18","-pix_fmt","yuv420p","-r","30","-fps_mode","cfr","-c:a","copy",str(candidate)]
