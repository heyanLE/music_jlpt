...
def main():
    ...
    authorization = {"userWording": "按当前审核版渲染", "createdAt": datetime.now(timezone.utc).isoformat(), "frameSha256": sha(PROJECT / "frames.json"), "sourceAndTemplateSha256": hashes}
    dump(PROJECT / "render" / "render-authorization.json", authorization)
    duration_ms = manifest["assets"]["music"]["durationMs"]
    output = FINAL / f"uchuu-no-fushigi-v2--study-current-v2--16x9--{RUN_ID}.mp4"
    ...
    entries, cursor, index = [], 0, 0
    for frame in frames:
        if cursor < frame["startMs"]:
            entries.extend([(blank, frame["startMs"] - cursor)])
        ...
        if local < frame["endMs"]:
            p = WORK / f"fg-{index:04d}.png"; render_foreground(frame, None, palette, p); entries.append((p, frame["endMs"] - local)); index += 1
    ...
    filters = f"[0:v]trim=duration={after_seconds},setpts=PTS-STARTPTS,crop=iw:trunc(ih*0.86/2)*2:0:0,scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H},fps={FPS}[v0];[2:v]trim=duration={tail_seconds},setpts=PTS-STARTPTS,fps={FPS}[v1];[v0][v1]xfade=transition=fade:duration={transition_seconds}:offset={transition_start}[bg];[1:v]fps={FPS},format=rgba[fg];[bg][fg]overlay=0:0:format=auto,fps={FPS},format=yuv420p[v]"
    command = ["ffmpeg", "-y", "-v", "error", "-i", str(SOURCE / "background.mp4"), "-f", "concat", "-safe", "0", "-i", str(concat), "-loop", "1", "-i", str(cover_bg), "-i", str(SOURCE / "music.flac"), "-filter_complex", filters, "-map", "[v]", "-map", "3:a:0", "-t", f"{duration_ms / 1000:.3f}", "-c:v", "libx264", "-preset", "medium", "-crf", "18", "-pix_fmt", "yuv420p", "-r", str(FPS), "-fps_mode", "cfr", "-c:a", "aac", "-b:a", "320k", "-movflags", "+faststart", str(output)]
