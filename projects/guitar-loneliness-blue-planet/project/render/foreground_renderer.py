...
class ForegroundRenderer:
    def __init__(self, root: Path, width: int, height: int):
        ...
        self.learning_aids = resolve_options(self.presentation)
        self.frames = {item["id"]: item for item in load(self.project / "frames.json")["frames"]}
        self.ordered_ids = [item["id"] for item in sorted(self.frames.values(), key=lambda f: f["startMs"]) if item["caption"]["japanese"].strip()]
    ...
    def measured_line(self, frame: dict) -> dict:
        """Reserve annotation ink width while keeping one source-index anchor map."""
        text = frame['caption']['japanese']
        spans, cursor = [], 0
        for card in frame.get('grammarCards', []):
            start = text.find(card['token'], cursor)
            if start < 0: raise RuntimeError(f"Unmapped card: {frame['id']} {card['token']}")
            cursor = start + len(card['token']); spans.append((start, cursor))
        runs, cursor = [], 0
        for ci,(start,end) in enumerate(spans):
            if cursor < start: runs.append((cursor,start,None))
            runs.append((start,end,ci)); cursor=end
        if cursor < len(text): runs.append((cursor,len(text),None))
        for size in range(self.px(70), self.px(53)-1, -1):
            font=ImageFont.truetype(FONT,size)
            measure=lambda value,ft=font: float(ft.getlength(value))
            positions=[0.]*(len(text)+1); ink_right=0.; boxes=[]
            for start,end,ci in runs:
                value=text[start:end]; width=measure(value)
                left,right=0.,width
                if ci is not None:
                    card=frame['grammarCards'][ci]
                    for key,ft in [('romaji',self.font(34)),('sourceWord',self.font(23))]:
                        if card.get(key):
                            aw=measure(card[key],ft)
                            left=min(left,(width-aw)/2-self.px(2))
                            right=max(right,(width+aw)/2+self.px(2))
                    for ruby in frame['caption'].get('furigana',[]):
                        rs=int(ruby.get('baseStart',-1))
                        if start <= rs < end:
                            rx=measure(text[start:rs]); rw=measure(ruby['reading'],self.font(28))
                            left=min(left,rx-self.px(2)); right=max(right,rx+rw+self.px(2))
                gap=self.px(8) if boxes else 0
                origin=ink_right+gap-left
                for j in range(start,end): positions[j]=origin+measure(text[start:j])
                positions[end]=origin+width
                boxes.append({'start':start,'end':end,'cardIndex':ci,'left':origin+left,'right':origin+right,'origin':origin,'lyricWidth':width})
                ink_right=origin+right
            if ink_right <= self.px(1728,'x'):
                return {'font':font,'width':ink_right,'positions':positions,'spans':spans,'boxes':boxes}
        raise RuntimeError(f"Shared lyric/romaji anchors exceed available width: {frame['id']}")
    ...
    def render(self, frame_id: str | None, active_index: int | None, output: Path | None, cover_only: bool = False, cover_visible: bool = True, countdown_value: int | None = None, *, lyric_only: bool = False, lyric_x_override: float | None = None) -> Image.Image:
        ...
        if cover_only:
            if cover_visible:
                cover_rect = ((self.width - self.px(220)) // 2, self.px(35, "y"), self.px(220), self.px(220))
        ...
        if not lyric_only: image.alpha_composite(Image.new("RGBA", image.size, veil))
        if cover_visible and not lyric_only:
            cover_rect = ((self.width - self.px(220)) // 2, self.px(35, "y"), self.px(220), self.px(220))
            image.alpha_composite(self.cover.resize((cover_rect[2], cover_rect[3]), Image.Resampling.LANCZOS), (cover_rect[0], cover_rect[1]))
        draw = ImageDraw.Draw(image); text = frame["caption"]["japanese"]
        layout = self.measured_line(frame)
        lyric_font = layout['font']; text_width = layout['width']
        lyric_x = (self.width - text_width) / 2
        positions = layout['positions']
        if lyric_x_override is not None: lyric_x = lyric_x_override
        if not lyric_only and self.learning_aids["neighbors"]["enabled"] and frame_id in self.ordered_ids:
            position = self.ordered_ids.index(frame_id)
            for adjacent, direction in ((position - 1, -1), (position + 1, 1)):
                if not 0 <= adjacent < len(self.ordered_ids): continue
                neighbor_id = self.ordered_ids[adjacent]
                neighbor_text = self.frames[neighbor_id]["caption"]["japanese"]
                neighbor_width = self.measured_line(self.frames[neighbor_id])['width']
                x = lyric_x - self.px(56, "x") - neighbor_width if direction < 0 else lyric_x + text_width + self.px(56, "x")
                neighbor = self.render(neighbor_id, None, None, lyric_only=True, lyric_x_override=x)
                neighbor.putalpha(neighbor.getchannel("A").point(lambda alpha: round(alpha * .60)))
                image.alpha_composite(neighbor)
            draw = ImageDraw.Draw(image)
        body, active = (255, 255, 255, 242), rgb(self.palette["activeTint"]) + (255,)
        parts = self.matched_parts(frame); cursor_x = lyric_x
        part_ranges, part_cursor = [], 0
        for part in parts:
            part_end = part_cursor + len(part["text"])
            part_ranges.append((part_cursor, part_end))
        ...
        if ''.join(part['text'] for part in parts) != text:
            raise RuntimeError(f"QRC text differs from render text: {frame_id}")
        for index, value in enumerate(text):
            self.outlined(draw, (lyric_x+positions[index], self.px(330, "y")), value, lyric_font, active if is_active(index,index+1) else body, 4)
        ...
        for ruby in frame["caption"].get("furigana", []):
            if "baseStart" in ruby:
                start = int(ruby["baseStart"]); end = start + len(ruby["base"])
            elif "start" in ruby and "end" in ruby:
            ...
            ruby_x = lyric_x + positions[start]
            token_span=next((span for span in card_spans if span and span[0]<=start<span[1]),(start,end))
            self.outlined(draw, (ruby_x, self.px(295, "y")), ruby["reading"], ruby_font, active if is_active(*token_span) else body, 2)
        ...
        for card_index, card in enumerate(frame.get("grammarCards", [])):
            ...
            token_x = lyric_x + positions[start]
            token_width = float(lyric_font.getlength(token))
            ...
            if card.get("sourceWord"):
                ...
                self.outlined(draw, (token_x + (token_width - annotation_width) / 2, self.px(295, "y")), card["sourceWord"], annotation_font, token_fill, 2)
            romaji = card.get("romaji", "")
            if romaji:
                romaji_font = self.font(34); romaji_width = draw.textbbox((0, 0), romaji, font=romaji_font)[2]
                self.outlined(draw, (token_x + (token_width - romaji_width) / 2, self.px(430, "y")), romaji, romaji_font, token_fill, 2)

        if lyric_only: return image
...
def main() -> None:
    ...
    binding_path = run / "run-binding.json"
    binding = {"authorizationSha256": sha(project / "render-authorization.json"), "canvas": canvas}
    if binding_path.exists() and load(binding_path) != binding:
        raise SystemExit("Run inputs changed; choose a new --run-id to prevent stale foreground reuse")
    if not binding_path.exists() and any(foreground_dir.iterdir()):
        raise SystemExit("Unbound existing foreground cache; choose a new --run-id")
    write(binding_path, binding)
    timeline_path = run / "foreground-timeline.json"
    subprocess.run([sys.executable, str(Path(__file__).with_name("build_foreground_timeline.py")), str(root), str(timeline_path), "--duration-ms", str(duration_ms)], check=True)
    ...
    for segment in timeline["segments"]:
        key = (segment["kind"], segment.get("frameId"), segment.get("activePartIndex"), segment["coverVisible"], segment.get("countdownValue"))
    ...
    if overlay_on:
        subprocess.run([sys.executable, str(Path(__file__).with_name("render_foobar_spectrum.py")), str(music), str(project / "palette.json"), str(spectrum_path), "--duration-ms", str(duration_ms), "--offset-ms", str(offset)], check=True)
    ...
    command += ["-filter_complex", ";".join(filters), "-map", "[video]", "-map", f"{music_index}:a:0", "-t", f"{duration_ms/1000:.6f}", "-c:v", "libx264", "-crf", "18", "-pix_fmt", "yuv420p", "-r", "30", "-fps_mode", "cfr", "-c:a", "copy", str(candidate)]
...
if __name__ == "__main__":
    raise SystemExit('Foreground-only project module. Use preview_proposals.py or a separately authorized custom composition entrypoint; the inherited fixed-preset main is not valid for this project.')
