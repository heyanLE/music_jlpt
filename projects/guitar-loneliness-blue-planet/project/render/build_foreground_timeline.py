...
def main() -> None:
    ...
    for start, end in zip(points, points[1:]):
        ...
        if count is not None:
            # Explicit project extension: reveal the complete first study frame
            # with the countdown, not throughout the spoken/musical prelude.
            if presentation.get('countdown', {}).get('showFirstLine', False):
                first = next((line for line in lines if line['text'].strip()), None)
                if first and mid < first['startMs']:
                    state = {'kind':'neutral','frameId':first['frameId']}
